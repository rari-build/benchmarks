import type { TodoItem } from '../functions'
import { getTodosList } from '../functions'

function renderGroceryItem(item: Readonly<TodoItem>) {
  ...
  const completedIndicator = item.completed ? <span className="text-green-600">✓</span> : null
...
export default function ShoppingList() {
  ...
  return (
    <div className="p-5 rounded-lg" data-component-id="shoppinglist">
      ...
      <ul className="space-y-2 mb-6">{groceries.map(item => renderGroceryItem(item))}</ul>
